# Copyright 2018-present, Tianyi Zhou (tianyizh@uw.edu)
# All rights reserved.
...
parser = argparse.ArgumentParser(description='Minimax Curriculum Learning (MCL)')
...
parser.add_argument('--alpha_rate', default=1.2, type=float,
...
parser.add_argument('--inflation_ratio', default=2.0, type=float,
                    help='inflation ratio of the uniform random subset (of size args.inflation_ratio * k0) to the final selected subset of size k0 (when use_random_subsample=True)')
 
parser.add_argument('--mod', default=0.2, type=float,
                    help='(initial) weight of hardness (when use_centrality=True, since the weight for centrality (alternative to diversity) is 1 - mod)')
parser.add_argument('--mod_rate', default=1.5, type=float,
                    help='multiplcation factor applied to mod every episode, we increase mod in MCL so mod_rate > 1')

parser.add_argument('--ema_decay', default=0.5, type=float,
                    help='decay factor of exponential moving average for mean_teacher')
...
parser.add_argument('--use_mean_teacher', action='store_true',
                    help='maintain a time moving average of the model (ensemble over time) as a teacher for consistency regularization (when args.consistency > 0)')
    ...
    for epoch in range(args.epochs):
        ...
        if epoch == args.schedule[min(schedule_idx + 1, len(args.schedule) - 1)]:
            ...
            if args.use_curriculum:
                ...
                # update trade-off weight between centrality and hardness
                batches_per_epoch = np.ceil(float(k0) / float(args.train_batch))
                args.mod *= max(1.2, args.mod_rate)
                args.alpha = min(1.2, args.alpha * max(1.2, args.alpha_rate))
        ...
        # warm starting episode (epochs from args.schedule[0] to args.schedule[1]): one full pass of all samples
        if (not args.use_curriculum) or epoch < args.schedule[1]:
            ...
            # store loss for next iteration's selection
        ...
        if args.use_curriculum and epoch >= args.schedule[1]:
            ...
            print('\nEpoch: [%d | %d] LR: %f, subset: %d' % (epoch + 1, args.epochs, state['lr'], k0))

            # sampling a subset according to all_loss_epoch (hardness) and centrality (alternative to diversity)
            if args.use_centrality:
                p = args.mod * (centrality_estimate / modular_estimate) * all_loss_epoch + centrality 
                if args.use_random_subsample:
                    rand_train_set = np.random.choice(n_train, k0, p = p / p.sum(), replace=False)
                else:
                    k1 = min(int(args.mk * n_train), int(args.inflation_ratio * k0))
                    rand_train_set = np.argpartition(p, -k1)[-k1:]
                    train_subset = np.random.choice(k1, k0, replace=False)
                    rand_train_set = rand_train_set[train_subset]    
                centrality_estimate, modular_estimate = centrality[rand_train_set].sum(), all_loss_epoch[rand_train_set].sum()     
            else:
                rand_train_set = np.random.choice(n_train, k0, p = all_loss_epoch / all_loss_epoch.sum(), replace=False)
            ...
            # update loss
            all_loss_epoch[rand_train_set] = loss_rest.cpu().numpy().astype(float)
